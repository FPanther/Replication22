...
For the purposes of analysis, we used only the phonotactic score that collapses vowel length distinctions, following the finding by Oh et al. (2020) that it correlates better with human phonotactic ratings. In the R code in this document, this measure is referred to as `p.scr`. In the rest of this document, the term "phonotactic score" refers the scores based on this length-neutralised model.
...
Neighbourhood density is a general measure of the number of minimal pairs that a given word type has, i.e. the number of other word types that are a Levenshtein edit distance of 1 from it. A host of studies have shown the processing of a word is affected by the neighbourhood density of a word.
...
Consequently, this item has a neighbourhood occupancy rate of $12/87 = 0.138$. In the R code in this document, this measure is referred to as `nm.nbr`.
...
# clmm(), which is used for ordinal regression modelling, has a bug that requires random terms to be below a certain size, or it returns an error. Here we rename two variables in the datasets so that the models run later in this document do not return errors.
identTask <- identTask %>%
  dplyr::rename(nm.nbr = norm.neighbors, n.phon = n.phonemes, p.scr = score.shortv)

wellFormTask <- wellFormTask %>%
...
#m1 <- clm(enteredResponse ~ c.(p.scr) * type * c.(nm.nbr) * bin * c.(n.phon), data=identTask)

#summary(m1) # five-way interaction is significant - keep all effects. Do a mixed effects model.

#m2 <- clmm(enteredResponse ~ c.(p.scr) * type * c.(nm.nbr) * bin * c.(n.phon) + (1|word) + (1|workerId), data=identTask)

#summary(m2) # score * type * nm.nbr * bin is significant. anything larger is not significant. n.phonemes is significant as a simple effect.

#m3 <- clmm(enteredResponse ~ (c.(p.scr) * type * c.(nm.nbr) * bin) + c.(n.phon) + (1|word) + (1|workerId), data=identTask)

#summary(m3) # four-way interaction not significant. score * type * bin is significant. No interaction involving nm.nbr is significant, but it is significant without interactions. Include it without interaction.
#anova(m2, m3) # m3 has a lower AIC

#m4 <- clmm(enteredResponse ~ (c.(p.scr) * type * bin) + c.(n.phon) + c.(nm.nbr) + (1|word) + (1|workerId), data=identTask)

#summary(m4) # nothing to remove.
#anova(m3, m4) # m4 has a lower AIC

#m5 <- clmm(enteredResponse ~ (c.(p.scr) * type * bin) + c.(n.phon) + c.(nm.nbr) + (1|word) + (1 + c.(p.scr) * type * bin + c.(n.phon) + c.(nm.nbr)|workerId), data=identTask)
#summary(m5) # model does not complete after running for several days - random terms are too complex. try a version with simple random slopes (i.e. no interactions)
#write_rds(m5, "docs/m5.rds")

#m6 <- clmm(enteredResponse ~ (c.(p.scr) * type * bin) + c.(n.phon) + c.(nm.nbr) + (1|word) + (1 + c.(p.scr) + type + bin + c.(n.phon) + c.(nm.nbr)|workerId), data=identTask)
#summary(m6) # three-way interaction is significant. create alternative model with binary contrast
#write_rds(m6, "docs/m6.rds")

#m6a <- clmm(enteredResponse ~ (c.(p.scr) * type * bin_b) + c.(n.phon) + c.(nm.nbr) + (1|word) + (1 + c.(p.scr) + type + bin_b + c.(n.phon) + c.(nm.nbr)|workerId), data=identTask)
#summary(m6a)
...
tibble(
  "Model"=c(
    "Model 6 (Three-Way Helmert Contrast)",
    "Model 6a (Binary Contrast)"
            ),
  "AIC" = c(
    AIC(m6),
    AIC(m6a)
  ...
  display_table(digits=1, highlight=c("AIC"), caption="Table 2: Comparison of AIC using different representations of the frequency bin factor. Model 6 is the best Model.")

remove(m6, m6a)
```

Table 3 summarises the best-fitting model. This model shows that the type of stimulus, phonotactic score, and frequency of stimuli (i.e. its stimulus bin) are significant predictors for the ratings that stimuli received: in general across frequency bins, ratings increased with phonotactic score, and words received higher ratings than non-words. Neighbourhood density and stimulus length (number of phonemes) both condition higher ratings. The former effect is likely to do with words with greater neighbourhood densities being more likely to activate words known to participants. The latter effect is likely to do with longer words appearing more Māori-like to participants. 

```{r Table3 Model Summary}
m6 <- read_rds("docs/m6.rds")
names(m6$coefficients) <- names(m6$coefficients) %>% str_replace_all("nm.nbr", "neighbourhood density") %>% str_replace_all("p.scr", "phonotactic score") %>% str_replace_all("n.phon", "number of phonemes")
clm_table(m6, caption="Table 3: Fixed Effects of the Word Identification Task Model.")
remove(m6)
```

Figure 4 shows the Predicted Rating of Stimuli by the three factors shown to interact significantly in the Word Identification Task Model: Phonotactic score, stimulus type, and frequency bin. This plot confirms the effects shown in figure 3: Real words in the high frequency bin show a different distribution from other stimuli: there is not a positive phonotactic slope, and there is a very high intercept.

```{r model results Figure 4, fig.width=8, fig.height=5, fig.cap="Figure 4: Three-Way Interaction in the Word Identification Task Model Between Phonotactic Score, Stimulus Type, and Frequency Bin"}
m6 <- read_rds("docs/m6.rds")
m6_data <- clm_plotdat(m6, c("p.scr", "type", "bin"), type="mean")
m6_data %>%
  mutate(
    bin = recode_factor(bin, "1" = "Low", "2" = "Mid", "3" = "High")
  ) %>%
...
#m6_lmer <- lmer(as.integer(enteredResponse) ~ (c.(p.scr) * type * bin) + c.(n.phon) + c.(nm.nbr) + (1|word) + (1 + c.(p.scr) + type + bin + c.(n.phon) + c.(nm.nbr)|workerId), 
#                data=identTask, 
#                control = lmerControl(optimizer = "bobyqa", optCtrl = list(maxfun=2e5)))
#write_rds(m6_lmer, "docs/m6_lmer.rds")
m6_lmer <- read_rds("docs/m6_lmer.rds")
m6_vif <- vif.mer(m6_lmer)
 names(m6_vif) <- names(m6_vif) %>% str_replace_all("nm.nbr", "neighbourhood density") %>% str_replace_all("p.scr", "phonotactic score") %>% str_replace_all("n.phon", "number of phonemes") %>% str_replace_all("[\\(\\)]", "") %>%  str_replace_all("c\\.", "(centered)")  %>% str_replace_all("(\\(centered\\))(.{1,})", "\\2 \\1")
...
```{r accuracy scores Figure 5, fig.width=5, fig.height=5, fig.cap="Figure 5: Histogram and Density Plot, Scaled to Kernel Density Estimation."}
...
Figure 6 shows the mean rating of stimuli in the Wellformedness Rating task by the phonotactic score of the stimuli. The results show a positive correlation between the rating of stimuli and their phonotactic score ($r = 0.60$).

```{r Figure 5, fig.width=6, fig.height=4, fig.cap="Figure 6: Mean Wellformedness Rating by Phonotactic Score. Dotted lines separate items into bins of equal size by phonotactic score."}
#Get mean ratings for each stimulus per bin
allBinsScored <- wellFormTask %>%
  group_by(word, bin=tert_count, p.scr) %>%
  summarise(
...
rating_by_phonotactics <- ggplot(allBinsScored, aes(x=p.scr, y=meanRating)) +
...
We analysed the data using (logit) ordinal regression, following the same stepwise procedure as outlined for the [Identification Task Ordinal Regression Model], except for this model we begin with random effects, given that we are testing fewer fixed effects. No terms were able to be removed.

```{r Wellformedness Rating Task Regression Model 1}

#modelAcc1 <- clmm(enteredResponse ~ c.(p.scr) * c.(Exp1.score) * c.(nm.nbr) + (1| workerId) + (1|word), data=wellFormTask)
#summary(modelAcc1a) # three-way interaction is not significant. Keep all two-way interactions.\

#modelAcc2 <- clmm(enteredResponse ~ c.(p.scr) * c.(Exp1.score) + c.(p.scr) * c.(nm.nbr) + c.(Exp1.score) * c.(nm.nbr) + (1|workerId) + (1|word), data=wellFormTask)
#summary(modelAcc2) # all interactions significant. add random slopes.

#modelAcc3 <- clmm(enteredResponse ~ c.(p.scr) * c.(Exp1.score) + c.(p.scr) * c.(nm.nbr) + c.(Exp1.score) * c.(nm.nbr) + (1 + c.(p.scr) * c.(nm.nbr)|workerId) + (1 + Exp1.score|word), data=wellFormTask)
#summary(modelAcc3) # exp1score * nm.nbr is not significant - remove.
#write_rds(modelAcc3, "docs/modelAcc3.rds")

#modelAcc4 <- clmm(enteredResponse ~ c.(p.scr) * c.(Exp1.score) + c.(p.scr) * c.(nm.nbr) + (1 + c.(p.scr) * c.(nm.nbr)|workerId) + (1 + c.(Exp1.score)|word), data=wellFormTask) 
#summary(modelAcc4) # model does not work


#modelAcc5 <- clmm(enteredResponse ~ c.(p.scr) * c.(Exp1.score) + c.(p.scr) * c.(nm.nbr) + (1 + c.(p.scr) + c.(nm.nbr)|workerId) + (1 + c.(Exp1.score)|word), data=wellFormTask) 
...
The final model is shown in Table 6. The model shows a significant effect of phonotactic score; as seen in [Wellformedness Rating Task Descriptive Statistics], stimuli are rated higher if they have higher phonotactic scores. The results also show a significant interaction between phonotactic score and neighbourhood density: non-words that have many real-word neighbours display less sensitivity to phonotactic score. We interpret this as a recognition effect: participants are more likely to recognise a neighbour in such circumstances and thus to be affected by the fact that the stimulus *is not* that neighbour.
...
modelAcc5 <- readRDS("docs/modelAcc5.rds")
 names(modelAcc5$coefficients) <- names(modelAcc5$coefficients) %>% str_replace_all("nm.nbr", "neighbourhood density") %>% str_replace_all("p.scr", "phonotactic score") %>% str_replace_all("Exp1.score", "identification task score")
...
#modelAcc5_lmer <- lmer(as.integer(enteredResponse) ~ c.(p.scr) * c.(Exp1.score) + c.(p.scr) * c.(nm.nbr) + (1 + c.(p.scr) + c.(nm.nbr)|workerId) + (1 + c.(Exp1.score)|word), data=wellFormTask)
#write_rds(modelAcc5_lmer, "docs/modelAcc5_lmer.rds")
modelAcc5_lmer <- readRDS("docs/modelAcc5_lmer.rds")
modelAcc5_vif <- vif.mer(modelAcc5_lmer)
 names(modelAcc5_vif) <- names(modelAcc5_vif) %>% str_replace_all("nm.nbr", "neighbourhood density") %>% str_replace_all("p.scr", "phonotactic score") %>% str_replace_all("Exp1.score", "identification task score") %>% str_replace_all("[\\(\\)]", "") %>%  str_replace_all("c\\.", "(centered)")  %>% str_replace_all("(\\(centered\\))(.{1,})", "\\2 \\1")
modelAcc5_vif %>%
  data.frame() %>%
  rownames_to_column(var = "Factor") %>%
  setNames(c("Factor", "VIF")) %>%
...
For ease of interpretation, we plot the interaction between performance in the Identification Task and sensitivity to phonotactic score in the Wellformedness Rating Task, using the same method described in the Detailed Materials and Method Supplement of Oh et al. (2020). This plot is shown in Figure 7: in general, participants with a higher Identification Task score will give ill-formed non-words lower scores and well-formed words higher scores, whereas participants with a lower Identification Task score will give all non-words ratings closer to 3 ("unsure").

```{r fig.width = 8, fig.height=6, fig.cap="Figure 7: Relationship Between Identification Task Score, Wellformedness Rating, and Phonotactic Score."}
modelAcc5 <- readRDS("docs/modelAcc5.rds")


fig3.mean <- clm_plotdat(modelAcc5, c("Exp1.score", "p.scr"), xlevels=list(p.scr=25, Exp1.score=c(0,1)), type="mean") %>%
  mutate(
    Exp1.score = factor(Exp1.score, labels=c("low (0 point difference)", "high (1 point difference)")) %>% fct_relevel("high (1 point difference)")
  ) %>%
  ggplot(., aes(x=p.scr, y=pred, color=Exp1.score, fill=Exp1.score)) +
...
fig3.dist <- clm_plotdat(modelAcc5, c("Exp1.score", "p.scr"), xlevels=list(p.scr=25, Exp1.score=c(0,1)), type="dist") %>%
  mutate(
     Exp1.score = factor(Exp1.score, labels=c("low (0 point difference)", "high (1 point difference)")) %>% fct_relevel("high (1 point difference)")
  ) %>%
  ggplot(., aes(x=p.scr, y=pred, color=Exp1.score, fill=Exp1.score)) +
